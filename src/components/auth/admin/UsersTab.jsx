...
export default function UsersTab() {
  ...
  const inputCls = "w-full rounded-lg px-3 py-2 text-[11px] text-white outline-none focus:ring-1 focus:ring-[#D32028] bg-[#05233E] border border-[#1e1e30]"
        ...
        <div className="rounded-xl border border-[#1e1e30] bg-[#0c0c18] p-4 mb-4">
          <form onSubmit={handleCreate} className="space-y-3">
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={busy} className="px-4 py-2 rounded-lg text-[11px] font-bold text-white bg-[#D32028] disabled:opacity-50">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold text-white"
                  style={{ background: u.role_id === 'admin' ? '#D32028' : u.role_id === 'tech' ? '#3b82f6' : '#374151' }}>
                  {(u.name || u.username).charAt(0).toUpperCase()}
                </div>
                <div>
                  {editingName === u.username ? (
                    <div className="flex items-center gap-1">
                      <input className="rounded px-2 py-0.5 text-[11px] text-white bg-[#05233E] border border-[#1e1e30] w-32"
                    ...
                    <p className="text-[12px] font-semibold text-white cursor-pointer hover:text-[#D32028]"
              ...
              <div className="flex items-center gap-2">
                {u.username !== 'cody' && (
                  <select className="rounded px-2 py-1 text-[10px] text-white bg-[#05233E] border border-[#1e1e30]"
            ...
            {resetPw === u.username && (
              <div className="mt-2 flex items-center gap-2 pl-11">
                <input type="password" placeholder="New password"
                  className="rounded px-2 py-1 text-[10px] text-white bg-[#05233E] border border-[#1e1e30] w-40"
