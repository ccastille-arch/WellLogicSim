...
function buildSnapshot(panelPayload, unitPayloads) {
  ...
  const siteDischarge = compressorDischarge.filter((value) => value != null).reduce((max, value) => Math.max(max, value), null)
...
export default function HalfmannTrending() {
  ...
  const refresh = useCallback(async () => {
    ...
    setSamples((current) => {
      const next = normalizeSamples([...current, snapshot]).filter((sample) => sample.timestampMs >= Date.now() - MAX_HISTORY_MS)
      return next
    })
  ...
  const chokeChartData = useMemo(() => {
    ...
    const chokeEvents = events
      .filter((event) => event.type === 'choke')
      .map((event) => ({ x: event.timestampMs, y: event.value }))
    if (chokeEvents.length) {
      datasets.push(makeMarkerDataset('Choke > 5%', colors.red, chokeEvents, 'triangle'))
    }

    const wellStateEvents = events
      .filter((event) => event.type === 'wellState')
      .map((event) => ({ x: event.timestampMs, y: event.value }))
    if (wellStateEvents.length) {
      datasets.push(makeMarkerDataset('Well online/offline', colors.gold, wellStateEvents, 'rectRot'))
    }

    return { datasets }
  }, [colors.gold, colors.red, downsampledSamples, events])

  const chokeChartOptions = useMemo(
  ...
  const compressorChartData = useMemo(() => {
    ...
    const compressorEvents = events
      .filter((event) => event.type === 'compressor')
      .map((event) => ({ x: event.timestampMs, y: event.value }))
    if (compressorEvents.length) {
      datasets.push(makeMarkerDataset('SP change', colors.white, compressorEvents, 'triangle'))
    }

    const dischargeEvents = events
      .filter((event) => event.type === 'discharge')
      .map((event) => ({ x: event.timestampMs, y: event.value }))
    if (dischargeEvents.length) {
      datasets.push(makeMarkerDataset('Discharge shift', colors.gold, dischargeEvents, 'circle', { yAxisID: 'y1' }))
    }
        ...
        callbacks: {
          ...
          label: (context) => {
            const matched = events.find((event) =>
              event.timestampMs === context.parsed.x && event.level === context.parsed.y,
            )
  ...
  const recentEvents = useMemo(() => {
    ...
    return events
      .filter((event) => event.timestampMs >= start && event.timestampMs <= end)
      .slice(-8)
      .reverse()
  ...
  return (
    <div className="min-h-screen bg-[#080810] text-white">
      <header className="border-b border-[#1a1a2a] bg-[#0c0c16] px-4 py-3 sm:px-5">
        <div className="mx-auto flex max-w-[1400px] flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <a href="/halfmann-view" className="rounded-full border border-[#1f3650] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#7dd3fc] hover:border-[#49d0e2] hover:text-white">
              Live View
            </a>
            <button
              onClick={() => {
      ...
      <main className="mx-auto flex max-w-[1400px] flex-col gap-4 px-4 py-5 sm:px-5 sm:py-6">
        {error && (
