...
const SLIDES = [
  ...
  { bg: '#0a0a14', content: ProblemSlide },
  { bg: '#0a0a14', content: CostSlide },
  { bg: '#0a0a14', content: SolutionSlide },
  { bg: '#0a0a14', content: HowItWorksSlide },
  { bg: '#0a0a14', content: PrioritySlide },
  { bg: '#0a0a14', content: TripResponseSlide },
  { bg: '#0a0a14', content: UnloadSlide },
  { bg: '#0a0a14', content: ROISlide },
  { bg: '#0a0a14', content: TimelineSlide },
  { bg: '#0a0a14', content: CTASlide },
]
...
export default function SlideDeck() {
  ...
  return (
    <div className="p-6 max-w-[1000px] mx-auto">
      ...
      {/* Slide */}
      <div className="relative bg-[#0a0a14] rounded-xl border border-[#222] overflow-hidden shadow-2xl"
        ...
        <div className="flex gap-1.5">
          {SLIDES.map((_, i) => (
            <button key={i} onClick={() => setSlide(i)}
              className={`w-2.5 h-2.5 rounded-full transition-colors ${i === slide ? 'bg-[#E8200C]' : i < slide ? 'bg-[#E8200C]/40' : 'bg-[#333]'}`} />
          ))}
        </div>
...
function TitleSlide() {
  return (
    <S>
      ...
      <div className="text-5xl text-white font-bold mb-4" style={{ fontFamily: "'Arial Black'" }}>Pad Logic TM</div>
...
function ProblemSlide() {
  return (
    <S className="items-start px-20">
      <div className="text-[#E8200C] text-sm font-bold uppercase tracking-wider mb-2">The Problem</div>
...
function CostSlide() {
  return (
    <S>
      <div className="text-[#E8200C] text-sm font-bold uppercase tracking-wider mb-2">The Cost</div>
      <div className="text-3xl text-white font-bold mb-8" style={{ fontFamily: "'Arial Black'" }}>Every Event Costs You Money</div>
      <div className="flex gap-8">
        {[
          ...
          <div key={i} className="text-center">
            <div className="text-4xl text-[#E8200C] font-bold" style={{ fontFamily: "'Arial Black'" }}>{item.value}<span className="text-lg text-[#888]">{item.unit}</span></div>
...
function SolutionSlide() {
  return (
    <S>
      ...
      <div className="text-3xl text-white font-bold mb-2" style={{ fontFamily: "'Arial Black'" }}>Pad Logic TM Handles It</div>
    ...
    <S className="items-start px-16">
      ...
      <div className="flex items-center gap-3 text-[12px] flex-wrap justify-center">
        {['Wells Produce', 'to', 'HP Scrubber', 'to', 'Gas Recirculates', 'to', 'Suction Header', 'to', 'Compressors', 'to', 'Discharge Header', 'to', 'Choke Valves', 'to', 'Back to Wells'].map((item, i) => (
          <span key={i} className={item === 'to' ? 'text-[#E8200C] text-lg' : 'bg-[#1a1a2a] px-3 py-1.5 rounded text-white font-bold'}>{item}</span>
        ))}
      </div>
      <div className="mt-6 text-[13px] text-[#888] text-center max-w-[600px]">
        Pad Logic controls the <span className="text-[#E8200C] font-bold">choke valves</span> - the injection point for each well.
...
function PrioritySlide() {
  return (
    <S>
      ...
      <div className="text-2xl text-white font-bold mb-6" style={{ fontFamily: "'Arial Black'" }}>Well Prioritization</div>
      <div className="flex gap-4 items-end mb-4">
        {[{ w: 'W1', pct: 100, pri: '#1' }, { w: 'W2', pct: 100, pri: '#2' }, { w: 'W3', pct: 35, pri: '#3' }, { w: 'W4', pct: 0, pri: '#4' }].map((item, i) => (
          <div key={i} className="text-center">
            ...
            <div className="w-20 h-32 bg-[#1a1a2a] rounded relative overflow-hidden border border-[#333]">
              <div className="absolute bottom-0 w-full transition-all" style={{ height: `${item.pct}%`, backgroundColor: item.pct > 80 ? '#22c55e' : item.pct > 30 ? '#eab308' : '#E8200C' }} />
            </div>
            <div className="text-[12px] text-white font-bold mt-1">{item.w}</div>
            <div className="text-[10px]" style={{ color: item.pct > 80 ? '#22c55e' : item.pct > 30 ? '#eab308' : '#E8200C' }}>{item.pct}%</div>
          </div>
        ))}
      </div>
...
function TripResponseSlide() {
  return (
    <S>
      <div className="text-[#E8200C] text-sm font-bold uppercase tracking-wider mb-2">Key Feature</div>
      ...
      <div className="grid grid-cols-2 gap-8 max-w-[700px]">
        <div className="bg-[#1a0808] rounded-lg p-4 border border-[#E8200C]/30">
          <div className="text-[#E8200C] font-bold text-sm mb-2">Without Pad Logic</div>
    ...
    <S>
      ...
      <div className="flex gap-3 items-center text-[12px]">
        {['Pressure Spike', 'to', 'Pad Logic Detects', 'to', 'Sales Valve Opens', 'to', 'Pressure Relieved', 'to', 'Shutdown Prevented'].map((item, i) => (
          <span key={i} className={item === 'to' ? 'text-[#eab308]' : 'bg-[#1a1a2a] px-3 py-2 rounded text-white font-bold'}>{item}</span>
        ))}
      </div>
    </S>
  )
    ...
    <S>
      ...
      <div className="flex gap-6 text-center text-[12px]">
        {[
          ...
          <div key={i} className="bg-[#1a1a2a] rounded-lg px-5 py-3">
            <div className="text-xl text-[#4fc3f7] font-bold" style={{ fontFamily: "'Arial Black'" }}>{v}</div>
...
function TimelineSlide() {
  return (
    <S>
      ...
      <div className="text-2xl text-white font-bold mb-8" style={{ fontFamily: "'Arial Black'" }}>From Order to Live in 6-8 Weeks</div>
        ...
        {[
          ...
          <div key={i} className="flex items-center gap-2">
            <div className="text-center">
              <div className="w-16 h-16 rounded-full border-2 border-[#E8200C] flex items-center justify-center bg-[#E8200C]/10">
                <span className="text-[10px] text-[#E8200C] font-bold">{step.week}</span>
              </div>
            ...
            {i < 4 && <div className="w-8 h-0.5 bg-[#E8200C]/50" />}
          </div>
        ))}
      </div>
    </S>
  ...
  return (
    <S>
      ...
      <div className="bg-[#111120] rounded-xl border border-[#E8200C]/30 px-10 py-6 text-center">
        <div className="text-2xl tracking-tight mb-1" style={{ fontFamily: "'Arial Black'", color: '#E8200C' }}>Pad Logic TM</div>
