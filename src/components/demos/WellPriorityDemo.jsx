  ...
  const triggers = [
    {
      ...
      description: 'Full gas, all wells at target',
      icon: 'OK',
      onClick: () => {
        sim.setTotalAvailableGas(maxGas)
        sim.state.compressors.forEach(c => sim.setCompressorStatus(c.id, 'running'))
      },
      active: sim.state.totalAvailableGas >= maxGas * 0.9,
    },
    { label: 'Gas Drops to 70%', description: 'Slight constraint - lowest priority well starts losing', icon: '70', onClick: () => sim.setTotalAvailableGas(maxGas * 0.7) },
    { label: 'Gas Drops to 50%', description: 'Moderate constraint - bottom half of wells cut back', icon: '50', onClick: () => sim.setTotalAvailableGas(maxGas * 0.5) },
    { label: 'Gas Drops to 30%', description: 'Severe - only top 1-2 wells get full injection', icon: '30', onClick: () => sim.setTotalAvailableGas(maxGas * 0.3) },
    { label: 'Trip Largest Compressor', description: 'Sudden capacity loss - watch priority kick in', icon: 'TRIP', onClick: () => sim.setCompressorStatus(0, 'tripped') },
    {
      label: 'Swap Top 2 Priorities',
      description: 'Reprioritize - gas shifts to new #1 well',
      icon: 'SWAP',
      onClick: () => {
    ...
    { label: 'Reverse All Priorities', description: 'Flip priority order - bottom becomes top', icon: 'REV', onClick: () => sim.setWellPriorities(sortedWells.map(w => w.id).reverse()) },
    {
