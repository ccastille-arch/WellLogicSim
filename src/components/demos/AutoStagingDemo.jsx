...
export default function AutoStagingDemo({ sim }) {
  ...
  const triggers = [
    { label: 'Full Pad â€” All Wells Online', description: 'Maximum demand â€” all compressors needed', icon: 'ðŸ“ˆ',
      onClick: () => {
    ...
    { label: 'Shut In Half the Wells', description: 'Operator shuts in low producers â€” excess capacity', icon: 'ðŸ“‰',
      onClick: () => {
        sim.state.wells.forEach((w, i) => sim.setWellDesiredRate(w.id, i < Math.ceil(sim.state.wells.length / 2) ? 150 : 0))
      }},
    { label: 'Single Well Only', description: 'Workover on pad â€” only one well producing', icon: '1ï¸âƒ£',
      onClick: () => {
        sim.state.wells.forEach((w, i) => sim.setWellDesiredRate(w.id, i === 0 ? 150 : 0))
      }},
    { label: 'Ramp Up Pad Gradually', description: 'New pad coming online â€” wells added one by one every 8 seconds', icon: 'â¬†ï¸',
      onClick: () => {
    ...
    { label: 'Night Mode â€” Reduce Rates', description: 'Operator lowers all well rates for overnight', icon: 'ðŸŒ™',
      onClick: () => sim.state.wells.forEach(w => sim.setWellDesiredRate(w.id, 80)) },
    { label: 'Morning Ramp â€” Full Rates', description: 'Morning crew brings rates back up', icon: 'â˜€ï¸',
      onClick: () => sim.state.wells.forEach(w => sim.setWellDesiredRate(w.id, 150)) },
    { label: 'Stop All Compressors', description: 'Full pad shutdown â€” everything stops', icon: 'ðŸ›‘',
      onClick: () => sim.state.compressors.forEach(c => sim.setCompressorStatus(c.id, 'stopped')) },
    { label: 'Restart Pad', description: 'Bring compressors back online', icon: 'ðŸŸ¢',
      onClick: () => sim.state.compressors.forEach(c => sim.setCompressorStatus(c.id, 'running')) },
    { label: 'Reset All', icon: 'â†©ï¸',
      onClick: () => {
        sim.state.compressors.forEach(c => sim.setCompressorStatus(c.id, 'running'))
        sim.state.wells.forEach(w => sim.setWellDesiredRate(w.id, 150))
        sim.setTotalAvailableGas(sim.state.maxGasCapacity)
  ...
  return (
    <DemoPage sim={sim} title="Automatic Compressor Staging"
      pitch="Pad Logic starts and stops compressors automatically based on pad demand. As wells come online or shut in, the system stages compressors up or down â€” matching capacity to demand without wasting fuel or leaving production on the table."
      triggers={triggers}>
