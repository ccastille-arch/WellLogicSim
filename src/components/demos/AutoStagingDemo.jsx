...
export default function AutoStagingDemo({ sim }) {
  ...
  const triggers = [
    { label: 'Full Pad — All Wells Online', description: 'Maximum demand — all compressors needed', icon: '📈',
      onClick: () => {
    ...
    { label: 'Shut In Half the Wells', description: 'Operator shuts in low producers — excess capacity', icon: '📉',
      onClick: () => {
        sim.state.wells.forEach((w, i) => sim.setWellDesiredRate(w.id, i < Math.ceil(sim.state.wells.length / 2) ? 150 : 0))
      }},
    { label: 'Single Well Only', description: 'Workover on pad — only one well producing', icon: '1️⃣',
      onClick: () => {
        sim.state.wells.forEach((w, i) => sim.setWellDesiredRate(w.id, i === 0 ? 150 : 0))
      }},
    { label: 'Ramp Up Pad Gradually', description: 'New pad coming online — wells added one by one every 8 seconds', icon: '⬆️',
      onClick: () => {
    ...
    { label: 'Night Mode — Reduce Rates', description: 'Operator lowers all well rates for overnight', icon: '🌙',
      onClick: () => sim.state.wells.forEach(w => sim.setWellDesiredRate(w.id, 80)) },
    { label: 'Morning Ramp — Full Rates', description: 'Morning crew brings rates back up', icon: '☀️',
      onClick: () => sim.state.wells.forEach(w => sim.setWellDesiredRate(w.id, 150)) },
    { label: 'Stop All Compressors', description: 'Full pad shutdown — everything stops', icon: '🛑',
      onClick: () => sim.state.compressors.forEach(c => sim.setCompressorStatus(c.id, 'stopped')) },
    { label: 'Restart Pad', description: 'Bring compressors back online', icon: '🟢',
      onClick: () => sim.state.compressors.forEach(c => sim.setCompressorStatus(c.id, 'running')) },
    { label: 'Reset All', icon: '↩️',
      onClick: () => {
        sim.state.compressors.forEach(c => sim.setCompressorStatus(c.id, 'running'))
        sim.state.wells.forEach(w => sim.setWellDesiredRate(w.id, 150))
        sim.setTotalAvailableGas(sim.state.maxGasCapacity)
