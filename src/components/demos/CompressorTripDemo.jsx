...
export default function CompressorTripDemo({ sim }) {
  ...
  const triggers = [
    ...comps.map(c => ({
      label: `Trip ${c.name}`,
      description: `Unexpected ${c.name} shutdown - engine fault`,
      icon: 'TRIP',
      onClick: () => sim.setCompressorStatus(c.id, 'tripped'),
      active: c.status === 'tripped',
    })),
    { label: 'Trip All Compressors', description: 'Total loss of compression - worst case', icon: 'ALL', onClick: () => comps.forEach(c => sim.setCompressorStatus(c.id, 'tripped')) },
    { label: 'Planned Shutdown C1', description: 'Operator takes C1 offline for maintenance', icon: 'STOP', onClick: () => sim.setCompressorStatus(0, 'stopped') },
    {
      label: 'Sequential Trip',
      description: 'C1 trips, then C2 trips 10s later',
      icon: 'SEQ',
      onClick: () => {
    ...
    { label: 'Bring C1 Back Online', description: 'Restart C1 after trip clear', icon: 'RUN', onClick: () => sim.setCompressorStatus(0, 'running') },
    { label: 'Restore All', description: 'All compressors running', icon: 'RESET', onClick: () => comps.forEach(c => sim.setCompressorStatus(c.id, 'running')) },
  ]
