  ...
  const triggers = [
    {
      ...
      description: `Suction stable at target (~${target} PSI)`,
      icon: 'OK',
      onClick: () => {
    ...
    {
      label: 'High Suction Pressure',
      description: 'Too much gas in header - compressors need to speed up',
      icon: 'HIGH',
      onClick: () => {
    ...
    },
    {
      ...
      description: 'Gas supply drying up - compressors slow to prevent trip',
      icon: 'LOW',
      onClick: () => sim.setTotalAvailableGas(sim.state.maxGasCapacity * 0.25),
    },
    {
      label: 'Sales Line Backpressure',
      description: 'Sales line pressure rises - gas backs up into suction',
      icon: 'BACK',
      onClick: () => {
    ...
    {
      label: 'Compressor Stagger Demo',
      description: 'Trip C1 to see stagger offsets in action - C2 reacts at different pressure',
      icon: 'STAG',
      onClick: () => sim.setCompressorStatus(0, 'tripped'),
    },
    {
      label: 'Suction Header Pre-Pack',
      description: 'Deliberately build header pressure before anticipated supply shortage',
      icon: 'PACK',
      onClick: () => sim.setStateField('suctionHeaderPressure', target + sim.state.suctionHighRange * 0.8),
    },
    {
      label: 'Ambient Temperature Spike',
      description: 'Hot day - gas temperature rises, density drops, suction pressure affected',
      icon: 'TEMP',
